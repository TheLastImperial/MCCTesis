...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \node(d)[rectangle, draw=black, minimum width=5cm, minimum height=1cm]
            [below=of c]
            {Extracción de información};

        \node(e)[rectangle, draw=black, minimum width=5cm, minimum height=1cm]
            [text width=4.5cm, align=center]
            [below=of d]
            {Validación de Extracción de la información};

        \node(f)[diamond, draw=black, minimum size=1.8cm]
            [right=of d]
            [text width=1.5cm, align=center]
            {Existen mas muestras};

        \node(g)[circle, draw=black, minimum size=1.5cm]
            [right=of f]
            {Fin};
        ...
        \draw[->] (e.east)  -- (f.south);
        \draw[->] (f.north) --
            node[text width=1cm,midway,above,align=center ] {Si}
            (c.east);

        \draw[->] (f) -- node[text width=1cm,midway,above,align=center ] {No} (g);
